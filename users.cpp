...
void insert_user(User *firstUser, User *nu){ //função recursiva que insere o usuario na lista //firstUser = Fist user
    if(!(firstUser->next)){
        firstUser->next = nu;
...
//------------------------------------Inserindo Usuário---------------------------------
        ...
        if(!List->head){
            ...
            if(find_cpf(List->head, nu->CPF) == false){   
...
bool find_cpf(User *firstUser, long int CPF){ // função recursiva que percorre a lista para verificar os CPFs dos usuários
    ...
    if(firstUser->CPF == CPF) return true;
    else check = find_cpf(firstUser->next, CPF);
